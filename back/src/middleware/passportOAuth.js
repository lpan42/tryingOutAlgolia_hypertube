    ...
    async (accessToken, refreshToken, profile, done) => {
      User.findOne({ oAuthId: profile.id }, async (err, user) => {
        ...
        if (!user) {
          ...
          if (!check_email) {
            user = new User({
              username: !(await User.findOne({ username: profile.username }))
                ? profile.username
                : profile.id,
              ...
              lastname: profile.displayName
                ? profile.displayName.split(" ")[1].toLowerCase()
                : null,
            ...
            if (!check_email) {
              user = new User({
                ...
                firstname: profile.data.first_name
                  ? profile.data.first_name.toLowerCase()
                  : null,
                lastname: profile.data.last_name
                  ? profile.data.last_name.toLowerCase()
                  : null,
                email: profile.data.email
