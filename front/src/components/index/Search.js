      ...
      return movies.map((movie, key) => (
        <div className='card' key={key}>
          ...
          >
            <img
              ...
              onError={(e) => {
                ...
                e.target.src = "/images/No_Picture.jpg";
              }}
            />
          ...
          <div className='content'>
            <a className='ui medium header' href={"/movie/" + movie.ImdbId}>
              {movie.Title}
              {inList(user.data.watched, movie.ImdbId) ? (
                <img
                  style={{ width: "15px", float: "right" }}
                  alt='watched'
                  src='http://localhost:3000/images/watched.png'
                />
              ) : null}
            </a>
            <div className='meta'>
          ...
          <div className='extra'>
            <Typography variant='subtitle1'>
              <StarIcon style={{ color: "#fdd835", margin: "0 10 0 0" }} />
              {movie.ImdbRating}
              {inList(watchLaterList, movie.ImdbId) ? (
                <BookmarkIcon
                  ...
                  onClick={() => setRemoveWatchLater(movie.ImdbId)}
                />
              ) : (
                <BookmarkBorderIcon
                  style={{
                  ...
                  onClick={() => setAddWatchLater(movie.ImdbId)}
                />
              )}
            </Typography>
          </div>
    ...
    <Fragment>
      <div className='ui search'>
        ...
        <input
          className='prompt'
          type='text'
          placeholder={language.filter.search}
      ...
      >
        <div style={{ width: 200, padding: "1.5em" }}>
          <Typography variant='body2'>
            {language.filter.year} {yearrange[0]} - {yearrange[1]}
          </Typography>
          <Slider
            value={yearrange}
            min={1900}
            max={2020}
            step={5}
            onChange={(e, settingyearrange) => {
              setYearrange(settingyearrange);
            }}
          />
          <Typography variant='body2'>
            {language.filter.rating} {ratingrange[0]} - {ratingrange[1]}
          </Typography>
          <Slider
            value={ratingrange}
            min={0}
            max={10}
            step={0.5}
            onChange={(e, settingratingrange) => {
              setRatingrange(settingratingrange);
            }}
          />
          <div>
            <FormControl
            ...
            >
              <Select value={genre} onChange={(e) => setGenre(e.target.value)}>
                ...
                <Button value='Adventure'>
                  {language.movietype.Adventure}
                </Button>
                <Button value='Animation'>
                  {language.movietype.Animation}
                </Button>
                <Button value='Biography'>
                  {language.movietype.Biography}
                </Button>
                <Button value='Comedy'>{language.movietype.Comedy}</Button>
                <Button value='Crime'>{language.movietype.Crime}</Button>
                <Button value='Documentary'>
                  {language.movietype.Documentary}
                </Button>
      ...
      <div className='ui centered grid'>
        {loading === true ? (
          ...
          <div className='ui doubling stackable cards'>{filmList}</div>
        )}
        {/* {suggPop || checkPage(page) === nbpages ? null : (
          <div>
            <ArrowDownwardIcon
              style={{ margin: "2em" }}
              onClick={() => setPage(checkPage(page + 1))}
            />
          </div>
        )} */}
      </div>
