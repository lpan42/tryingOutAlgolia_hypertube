...
const Movie = ({ match }) => {
  ...
  const addWatchLater = async () => {
    ...
    try {
      const result = await axios.post(
        `/movie/watchlater/add/${match.params.imdb_id}`
      );
  ...
  const removeWatchLater = async () => {
    ...
    try {
      const result = await axios.post(
        `/movie/watchlater/remove/${match.params.imdb_id}`
      );
      setSuccess(result.data.success);
      loadUser();
    } catch (err) {
    ...
    <Fragment>
      ...
      <div className={classes.movieInfo}>
        ...
        <div className={classes.movieContent}>
          ...
          <Typography variant='subtitle2'>
            {lang.movie.imdbrating}: {movieInfo.ImdbRating} / 10
          </Typography>
          <Typography variant='subtitle2'>
            {lang.movie.country}: {movieInfo.Country}
          </Typography>
          <Typography variant='subtitle2'>
            {lang.movie.director}: {movieInfo.Director}
          </Typography>
          <Typography variant='subtitle2'>
            {lang.movie.actors}: {movieInfo.Actors}
          </Typography>
          <br></br>
          <Typography variant='subtitle2'>{lang.movie.overview}:</Typography>
          <Typography variant='subtitle2'>{movieInfo.Plot}</Typography>
        </div>
      </div>
  ...
  return (
    <div>{invalidId ? <Typography>{invalidId}</Typography> : moviePage}</div>
  );
};
