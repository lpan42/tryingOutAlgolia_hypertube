      ...
      showComment.push(
        <List key={key} className={classes.singleComment}>
          <Typography variant='body1'>
            <a href={"/account/" + comment.userId}>{comment.username}</a> @{" "}
            {moment(comment.time).format("DD-MM-YYYY HH:mm")}
          </Typography>
    ...
    <div>
      <div className={classes.commentDiv}>
        ...
        <form className={classes.chatBox}>
          <TextField
            placeholder={language.comment.leavecomment}
          ...
          <IconButton
            type='submit'
            color='primary'
            onClick={(e) => sendNewComment(e)}
          >
            <SendIcon />
          </IconButton>
        </form>
        <div className={classes.comments}>{showComment}</div>
      </div>
    </div>
  );
};
