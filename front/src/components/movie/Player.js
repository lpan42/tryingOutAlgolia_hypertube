...
const MoviePlayer = ({ imdb_id }) => {
  ...
  useEffect(() => {
    if (currentSub === "English") setSubSrc(subPath.en);
    if (currentSub === "Français") setSubSrc(subPath.fr);
    if (currentSub === "No subtitle") setSubSrc("");
    //eslint-disable-next-line
  }, [currentSub]);
  //
  useEffect(() => {
  ...
  const selectTorrent = async (e) => {
    ...
    setVideoSrc(
      `http://localhost:8000/movie/stream/${imdb_id}&${
        singleMovie.Torrents[e.currentTarget.value].provider
      }&${singleMovie.Torrents[e.currentTarget.value].quality}`
    );
    handleMovieClose();
  ...
  singleMovie &&
    singleMovie.Torrents.map((torrent, key) => {
      torrentSelection.push(
        <MenuItem key={key} value={key} onClick={(e) => selectTorrent(e)}>
          {torrent.quality}
          {torrent.provider}
        </MenuItem>
      );
        ...
        <Paper>
          <ClickAwayListener onClickAway={handleSubClose}>
            <MenuList autoFocusItem={openSub}>
              {subPath.en === null ? null : (
                <MenuItem value='1' onClick={(e) => selectSub(e)}>
                  English
                </MenuItem>
              )}
              {subPath.fr === null ? null : (
                <MenuItem value='2' onClick={(e) => selectSub(e)}>
                  Français
                </MenuItem>
              )}
              <MenuItem value='0' onClick={(e) => selectSub(e)}>
                No Subtitle
              </MenuItem>
            </MenuList>
          </ClickAwayListener>
      ...
      <Popper open={openMovie} anchorEl={chooseMovie.current}>
        <Paper>
          <ClickAwayListener onClickAway={handleMovieClose}>
            <MenuList autoFocusItem={openMovie}>{torrentSelection}</MenuList>
          </ClickAwayListener>
        </Paper>
      </Popper>
    </div>
